    ...
    <ul>
      ...
      <li>
        <el-radio-group v-model="choose" class="ml-4" @change="change">
          <el-radio label="1" size="large">个人下架的</el-radio>
          <el-radio label="2" size="large">平台下架的</el-radio>
        </el-radio-group>
      </li>
              ...
              <el-col :span="2" :offset="3">
                <ul>
                  <div v-if="choose === '1'">
                    ...
                    <li>
                      <el-button class="margin_top_25" type="success" @click="on(item.goodsId)"> 上架</el-button>
                    </li>
                  </div>
                </ul>
              </el-col>
...
export default {
  ...
  setup(props) {
    ...
    // 下架
    const on = async (goodsId) => {
      try {
        const {data} = await onGoods(goodsId)
        if (!data.ok) {
          return;
        }
      } catch (e) {
